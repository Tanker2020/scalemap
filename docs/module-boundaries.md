...
### H. Utility dock (Diagnostics + Reports) — 2026-07-02
...
**Blast radius:** low. `UtilityDock` has 1 caller (`App.tsx`). `DiagnosticsPanel`/`ReportsPanel` are each imported only by `UtilityDock.tsx` now (previously imported directly by `App.tsx`).
...
6. **Right-edge panel/IA work** (new panels, further discoverability fixes) → §1H, isolated to `dock/` + the panel-open fields in `ui.store.ts`. Read the "why flex child, not `position: fixed`" note in §1H before adding another right-edge panel.
