...
| `src/app/toolbar/Toolbar.module.css` | Shared by both files (`FileMenu.tsx` imports `styles` from here rather than getting its own CSS module). Holds the Task 2 dropdown/submenu classes (`.dropdownItem:disabled`, `.dropdownSubWrap`, `.submenuChevron`, `.dropdownSubmenu`) plus the Task 4 narrow-width safety net: `.toolbarMain` carries `overflow-x: hidden` as a hard "never scrolls" guarantee, backed by two `max-width` breakpoints that shed low-priority content before that guarantee would otherwise clip anything — `max-width: 1000px` hides `.undoRedoGroup`, `max-width: 860px` hides `.scriptPill`. `.themeToggleFixed` is unaffected by either breakpoint since it lives outside `.toolbarMain` in the DOM |
...
**Known gap found during the Task 5 verification pass (not yet fixed):** the two `Toolbar.module.css` breakpoints do not fully guarantee the Simulate button (or its settings chevron) stays visible/clickable at every width, despite `.toolbarMain`'s `overflow-x: hidden` intending exactly that. Reproduced on a clean `npm run dev` reload at 1440×900 viewport, Load Balanced Cluster template loaded: in the ~1001–1030px window (just above the 1000px breakpoint, before Undo/Redo hide), the Simulate split-button's settings chevron is laid out partly past `.toolbarMain`'s clipped right edge and overlaps the fixed-position theme toggle, which silently steals the click (`elementFromPoint` at the chevron's center resolves to `.themeToggleFixed`, not the chevron). At 900px the same overlap/clip recurs. At 700px it's worse — the entire Simulate button (and chevron) lies fully past the clipped boundary and is not rendered on screen at all, `Diagnostics` truncates to icon-only, yet `.toolbarMain`'s `scrollWidth` (859px) still exceeds its clipped `clientWidth` (638px). The breakpoints only shed Undo/Redo and the ScaleScript pill; they don't shed or reflow enough at these widths to keep the Simulate button inside the clip boundary, and `.themeToggleFixed`'s fixed positioning was never coordinated against `.toolbarMain`'s content width to guarantee zero overlap. This needs a follow-up fix (e.g. a third breakpoint that shrinks/hides the Panels group's text labels, or reserving fixed right-hand space for the theme toggle in the breakpoint math) before the "toolbar never scrolls or clips" guarantee this section previously claimed can be considered true at all widths.
