...
| `src/app/store/simulation.store.ts` | Wave-1 additions: `setFault`/`setOutage` actions, `impairmentMemo` selector; Task 13 added `setPartition(fault)`/`healPartition(index)` — thin delegations to the Task-12 engine facade methods of the same name, no store-side bookkeeping (the facade owns the active-partitions list); hub file, sequence with other Wave-1 mutators |
