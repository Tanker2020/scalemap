...
| `src/lib/worldEngine/types.ts` | Wave-1 additions: `FaultInjection` field on `EngineState`/`WorldEngineApi`, `impairmentMemo` on per-step state; Task 12 added a MINIMAL `WorldEngineApi.setPartition`/`healPartition` pair (thin wrappers over `faults.ts`'s `addPartition`/`removePartition`) ahead of Task 13; Task 13 confirmed the pair as-is at the time (no extension needed then — its store delegation and analysis rule only needed `setPartition`/`healPartition` at those signatures) and consumed it, no duplication. Task 13's review round added `InstanceMetrics.effectiveRole?: PlacementRole` (the LIVE promotion-aware role, `failover.ts`'s `effectiveRoleResolver` output — the static `compiled.instances[id].role` never reflects a partition-induced promotion, which only ever mutates the engine's internal `promotedAt`) — see `metrics.ts`/`index.ts` below for how it's populated. A LATER final-review fix (I3, 2026-08-02) deliberately broke `healPartition`'s signature from `(index: number)` to `(id: string)` — see `.superpowers/sdd/contract-drift.md` for the rationale (array position wasn't a safe identity for a live-mutable partitions list); this is the one intentional non-additive exception to this file's otherwise additive-only convention, logged there rather than silently reshaping the type |
| `src/lib/worldEngine/metrics.ts` | Task 13 (review fix): `buildBatch` gained a trailing optional `roleOf?: (id: InstanceId) => PlacementRole` param, populating the new `InstanceMetrics.effectiveRole` — same additive-optional-param convention as `starved`/`connProfiles`/`effectiveCpuMsByInstance`/`leakAccumMb` above it; absent ⇒ `effectiveRole` stays undefined, every existing direct-`buildBatch` caller/test unchanged |
| `src/app/store/simulation.store.ts` | Wave-1 additions: `setFault`/`setOutage` actions, `impairmentMemo` selector; Task 13 added `setPartition(fault)`/`healPartition(id)` — thin delegations to the Task-12 engine facade methods of the same name, no store-side bookkeeping (the facade owns the active-partitions list); `healPartition` was re-addressed from index to id in the I3 final-review fix (see `.superpowers/sdd/contract-drift.md`), and this store's own local `partitions` mirror follows suit; hub file, sequence with other Wave-1 mutators |
...
| `src/lib/worldEngine/index.ts` | Task 12: the step function now computes a per-(observer region, target region) directional health view (`regionSeesRegionDown`) via `impairmentFor(fromIds=target, toIds=observer, partitions)` — an observer sees a target as down iff the TARGET→OBSERVER leg is impaired (the response-leg direction) — batched through ONE `runHealthChecks` call alongside the existing global region/AZ scopes. Falls back to the target's own non-directional `checkFailed` when no partition is active (byte-identical to pre-Task-12 behavior). A SEPARATE, narrower promotion path (right after the existing `promoteReplicas` call) handles the case `promoteReplicas`' same-region-only sibling rule can never reach: a replica placed in a DIFFERENT region than every authored primary of its blueprint self-promotes when its own region cannot directionally reach any of those primaries' regions, and fails back once reachability returns — this is DELIBERATELY independent of, and does not modify, `promoteReplicas`'/`failover.ts`'s same-region sibling-selection semantics. **Final-review fix wave (audit C1/I4, see the "Fault & Scenario Substrate — Wave 1" §'s FEAT-002 bullet below for the full writeup):** the original version's only guard (`crossRegionPrimaries.length === 0`) failed to exclude a replica that ALSO had a same-region authored primary sibling, and it read `alreadyPromoted` off the SAME `promotedAt` map `promoteReplicas` writes — the two together produced a perpetual promote/failback flap whenever such a replica's same-region primary died (zero partitions needed). Fixed by (a) precomputing eligibility once at `start()` (`EngineState.crossRegionOrphanReplicaIds` — a replica with a same-region primary is excluded structurally) and (b) tracking this block's OWN promotions in a dedicated `FailoverState.isolationPromotedAt` map, so its failback can only ever reverse a promotion it made itself. Task 13 (review fix): the sole `buildBatch` call site now passes the already-memoized `roleOf` (§6's `s.roleResolver`, rebuilt only when `promotedAt`'s contents change) as `buildBatch`'s new `roleOf` param — no new computation, just publishing a resolver the step loop already builds for flow routing. **Task 18 (FEAT-003, first scenario-timeline engine integration):** `start()` sorts `doc.scenario?.steps` by `atMs` once into `EngineState.scenarioSteps`/`scenarioCursor` (empty/0 for no scenario); `runStep`'s new top-of-step `while` loop advances the cursor and applies every step whose `atMs <= simMs` exactly once (sorted order breaks same-`atMs` ties), dispatching through a new `applyScenarioAction` that reuses `api.setFault`/`api.setPartition`/`api.healPartition` verbatim for `inject-fault`/`clear-fault`/`partition`/`heal-partition` (no duplicated fault/partition wiring) and writes `EngineState.demandOverlay: Map<PopulationId, {multiplier; targetMultiplier; rampStartMs; rampSec}>` for `demand-multiplier` (every population, target = `action.factor`) / `set-population-rps` (one population, target = `action.peakRps / population.peakRps`, guarding a zero baseline to `0`) — both seed `multiplier` from the CURRENT ramped-effective value (`effectiveOverlayMultiplier`, from the new shared `rampMath.ts` — see its own row below) rather than resetting to 1, so a second action mid-ramp continues smoothly. **Task 19 (FEAT-003, demand overlay consumption):** `demand.ts`'s `populationDemandRps` now DOES read this map — threaded through as its new optional `demandOverlay` param at the `runStep`'s `demandByPop` loop's call site (`populationDemandRps(pop, simMs, s.rng, stepMs, ds, s.demandOverlay)`) — scaling the diurnal mean by the population's current ramped overlay multiplier (via the same `rampMath.ts` helper `index.ts` uses) before the Poisson draw; a population absent from the map gets multiplier 1 (byte-identical to pre-scenario worlds). Also: `doc.scenario?.seed`, when present, REPLACES `createWorldEngine`'s constructor-seed default for both `state.rng` and the tracer's rng (`effectiveSeed = doc.scenario?.seed ?? seed`) rather than adding a second independent rng source — see `.superpowers/sdd/contract-drift.md`'s 2026-08-02 entry for the new `scenario_step_applied` `EngineEventKind` and the determinism test that pins this down |
| `src/lib/worldEngine/rampMath.ts` (Task 19, FEAT-003) | A tiny dependency-free leaf module holding the ONE ramp-interpolation formula for a demand-overlay entry: `DemandOverlayEntry` (`{multiplier; targetMultiplier; rampStartMs; rampSec}`, the same shape `EngineState.demandOverlay`'s values carry) and `effectiveOverlayMultiplier(entry, simMs)` — linear interpolation from `multiplier` to `targetMultiplier` over `rampSec`, clamped to `[0,1]` progress, returning `targetMultiplier` once the ramp completes (or immediately if `rampSec<=0`). Imported by BOTH `index.ts`'s write side (`applyScenarioAction`'s `demand-multiplier`/`set-population-rps` branches, seeding a new action's starting multiplier from wherever an in-flight ramp currently sits) and `demand.ts`'s read side (`populationDemandRps`, computing the multiplier to apply before the Poisson draw) — the two MUST compute "the overlay's current effective multiplier at simMs" identically, or a demand-shaping scenario step firing mid-ramp would visibly discontinuity-jump. Extracted to its own file rather than living in either `index.ts` or `demand.ts` specifically to avoid introducing a new import direction: `index.ts` already imports from `demand.ts` and never the reverse, so a shared leaf both can import from keeps that invariant intact |
| `src/app/world/panels/ScenarioPanel.tsx` (Task 20, FEAT-003, new file) | The scenario-timeline AUTHORING surface (world-scope `scenario` tab, registered alongside `routes`/`traffic` in `ui.store.ts`'s `PanelTab`/`scope.ts`'s `WORLD_TABS`/`WorldPanel.tsx`'s `TAB_LABELS`+render switch). No `doc.scenario` yet ⇒ a single "+ Create scenario" button (`setScenario({..., steps: []})`); once one exists, renders a label/duration/seed header, an `<ScenarioRuler>` (exported — the same component `SimControls` could reuse, though it currently opts for a simple percentage chip instead, see below) with draggable step markers, the step list (edit `atMs`/note inline, delete), and an `AddStepForm` covering all six `ScenarioAction` variants (entity pickers scoped to the doc's actual servers/azs/regions/managedServices/populations, mirroring `PartitionsSection.tsx`'s `entityOptions` idiom but with plain `<select>`s). **Edit-lock direction is the ORDINARY one, not `ChaosControl`/`PartitionsSection`'s inverse escape-fieldset pattern** — authoring a scenario is configuration, not a live chaos action, so this file is mounted as a normal tab body inside `WorldPanel.tsx`'s ambient `<fieldset disabled={running}>` and uses plain native `<button>`/`<input>`/`<select>` throughout, no `Pressable`/escape machinery. The one non-native control — the ruler's draggable marker `<div>` — is NOT protected by the ambient fieldset (fieldset-disabling only touches form-associated elements), so its own `onMouseDown` explicitly no-ops `if (running)`. Writes exclusively through `world.store.ts`'s `setScenario`/`addScenarioStep`/`removeScenarioStep`/`updateScenarioStep` (Task 17, undo/dirty for free); never calls the engine or `simulation.store.ts` directly. `ScenarioPanel.test.tsx` (10 cases) covers the empty state, step add/delete, the running edit-lock tooltip, and the ruler's progress fill |
| `src/app/store/simulation.store.ts` (Task 20 addition) | Gained `selectScenarioProgressMs` — a one-line selector (`s.latestBatch?.simMs ?? 0`) so `ScenarioPanel`'s ruler and `SimControls`'s progress chip don't each reach into `latestBatch.simMs` independently. **Deliberately did NOT add `runScenario()`/`stopScenario()` actions** despite earlier planning language suggesting them: `doc.scenario`'s steps are already applied engine-side purely from its presence (Task 18/19's `scenarioSteps`/`scenarioCursor` in `worldEngine/index.ts`, triggered by the EXISTING `start()`) — "running a scenario" IS running the engine, so a parallel action would just be a second name for `start()`/`stop()` with nothing store-side of its own to do |
| `src/app/world/SimControls.tsx` (Task 20 addition) | The Simulate button now reads `doc.scenario ? 'Run scenario' : 'Simulate'` — same `onClick={() => start(doc, compiled)}`, no new action, since `start()` already applies scenario steps once `doc.scenario` is present. A `scenario NN%` chip (styled like the existing `degraded`/`warmingUp` chips) renders only while `running && doc.scenario`, reading `selectScenarioProgressMs` against `doc.scenario.durationMs`. No separate run/stop-scenario control exists — the header has exactly one start affordance and one stop affordance (`End`), scenario or not |
| `src/lib/vault/exampleWorlds.ts` (Task 20 addition) | Three of the four vault entries now carry an authored `doc.scenario` (attached, not new vault entries — the vault's 4-entry/id-order contract in `exampleWorlds.test.ts` is unchanged): `multi-region-failover` gets "regional failure" (region down → demand-multiplier retry-storm bump → clear-fault recovery, using the world's own populations), `three-tier` gets "creeping memory leak" (a `memory-leak` fault escalating — via a SECOND `inject-fault` on the same id, which REPLACES the first in the single-fault-per-id `activeFaults` map, not stacks alongside it — into `error-inject`, then `clear-fault`), `event-driven` gets "Black Friday ramp" (three `demand-multiplier` waves). `doc.scenario` participates in neither `compileWorld` nor any analysis rule (verified by grep before attaching), so this leaves every vault entry's existing zero-findings/12-findings contract untouched |
...
| `src/lib/analysis/rules/structural.ts` (Task 1, 6 rules; +1 Wave-1 Task 13) | `single-az-region`, `no-failover-region`, `replicas-colocated`, `dependency-cycle`, `deep-sync-chain`, `unused-managed-service` — read `compiled.instances`/`compiled.routing.populationRegionOrder`/`doc.blueprints` only. Wave-1 Task 13 added `split-brain-risk` (FEAT-002): flags a `blueprintId\|regionId` cluster (same convention as `failover.ts`'s `promoteReplicas`, derived inline — no shared helper exists) with >1 instance whose EFFECTIVE role is `'primary'` — reads `lastBatch?.instances[id]?.effectiveRole ?? compiled.instances[id].role` (review-round fix: the rule originally read only the STATIC `ServiceInstance.role`, which never reflects a partition-induced promotion since `promoteReplicas` only mutates the engine's internal `promotedAt`, never the doc — `effectiveRole` is the new additive-optional `InstanceMetrics` field, §Wave-1/`worldEngine/types.ts` above, populated from the step loop's already-memoized `roleResolver`), gated on `doc.blueprints[bpId].stateful` (mirrors `replicas-colocated`'s gate — ungated, it false-fired on any multi-placement STATELESS tier, since `role` defaults to `'primary'` for every placement regardless of blueprint; caught by the vault `exampleWorlds.test.ts` zero-findings assertions). Known accepted gap: the region-scoped cluster key means a GENUINE cross-region split brain (primary in region A untouched, self-promoted replica in region B) is never grouped into one cluster and is not detected — out of scope for Task 13 |
...
| `src/lib/worldEngine/types.ts` | Adds `FaultKind`/`FaultSpec`/`FaultScope`/`LinkEndpoint`/`PartitionFault` (the frozen types `faults.ts` operates on), `WorldEngineApi.setFault`/`setOutage`/`setPartition`/`healPartition`, and five new `EngineEventKind`s (`fault_injected`/`fault_cleared`/`partition_started`/`partition_healed`/`scenario_step_applied`) — additive-only at the time of this Wave-1 pass, with one later deliberate exception: the I3 final-review fix broke `healPartition`'s signature from `(index: number)` to `(id: string)` (see `.superpowers/sdd/contract-drift.md`), so this table entry's own contract is no longer a blanket additive-only claim |
...
- **FEAT-002 (Tasks 9-14):** `LinkEndpoint`/`PartitionFault`/`impairmentFor` (Tasks 9-10),
  ...
  views (Task 14).
  - **A SECOND, independent promotion path lives in `index.ts` (Task 12), right after the
    `promoteReplicas`/`failbackPromotions` calls (`failover.ts`).** `promoteReplicas` only ever
    promotes a sibling replica in the SAME region as a down primary; this second block handles the
    case it structurally cannot reach — a replica with NO same-region authored primary sibling but
    WITH an authored primary for its blueprint in another region self-promotes when its own region
    loses directional reachability to every region hosting one of those primaries (genuine
    cross-region split-brain), and fails back once reachability returns. **Ownership invariant
    (added in the final-review fix wave, audit finding C1, after this originally flapped
    promote/failback every step for a replica that ALSO had a same-region primary sibling):**
    (1) eligibility is precomputed once at `start()` into
    `EngineState.crossRegionOrphanReplicaIds` — a replica with a same-region authored primary is
    excluded structurally and can never reach this block, full stop, regardless of partition
    state; (2) promotion ownership is tracked in a DEDICATED `FailoverState.isolationPromotedAt`
    map, separate from `promoteReplicas`'/`failbackPromotions`' own `promotedAt` bookkeeping — this
    block only ever fails back a placement it ITSELF promoted (checked via
    `isolationPromotedAt.has`), never one `promoteReplicas` promoted for an unrelated same-region
    cluster. The block still WRITES into the shared `promotedAt` map too (so
    `effectiveRoleResolver`'s routing overlay sees the promotion), which is safe precisely because
    of (1): an orphan replica's blueprint+region has zero authored primaries, so
    `failbackPromotions`'s same-region-cluster-keyed scan finds nothing there and never touches it.
    See `index.test.ts`'s `sameRegionPlusCrossRegionPrimaryFixture` for the permanent regression
    fixture (same-region primary + same-region replica + a separate cross-region primary, primary
    killed, zero partitions — asserts ≤1 `replica_promoted` and zero `primary_failback` over 20
    simulated seconds).
...
|---|---|---|
| `src/lib/worldEngine/cache.ts` | FEAT-004 | Pure, dependency-free hit-ratio/miss-fraction resolver (`effectiveHitRatio`/`effectiveMissFraction` functions, accounting for cache warmth ramp). Exports two functions consumed by exactly two call sites: (1) `flows.ts`'s dependency loop (line ~713), which reads `input.cacheAsideIndexByDepId?.get(dep.id)` to resolve a dependency's cache-aside sibling's config and warm-key, then applies the sibling's miss fraction to reduce the dependency's admitted rps; (2) `metrics.ts`'s per-instance loop (line ~212), which reads `s.warmSinceMs.get(inst.id)` to compute a cache instance's observed hit ratio for publication in the 1 Hz batch. Introduced: `CacheConfig` type (in `world/types.ts`, Task 1), optional `cacheConfig?: CacheConfig` on `ServiceBlueprint` and `ManagedService` (Task 1), optional `cacheAsideVia?: string` on `BlueprintDependency` (Task 1), and optional `cacheHitRatio?: number` on `InstanceMetrics` (Task 4, published per-step). Start-time indexes (built once, read many per-step): `EngineState.hasAnyCache` (boolean guard, Task 3), `EngineState.warmSinceMs` (cache cold-cycle track, Task 5), `EngineState.cacheAsideIndexByDepId` (dependency→sibling cache mapping, Task 3 Step 6 — prevents O(deps) re-walk in flows.ts per-step dependency loop). **Audit final-review fix**: `index.ts`'s `doSetFault` originally reset `warmSinceMs`/emitted `cache_cold` whenever a fault was CLEARED (`spec === null`) on a scope with resident cache instances, regardless of which fault kind was actually being cleared — since `faults.ts` keeps exactly one spec per `${scope}:${id}` key, clearing a `latency-add`/`cpu-brownout`/`memory-leak`/`error-inject`/`disk-stall` fault falsely fired a cold-cache thundering herd even though the instance never restarted. `doSetFault` now captures `wasDown = spec === null && s.faults.active.get(key)?.kind === 'down'` BEFORE `setFaultPure` deletes the entry, and gates the warmth reset on `wasDown` — only an actual `down`-fault clear (a real restart) resets warmth. Covered by two `index.test.ts` cases in the FEAT-004 describe block: clearing a `cpu-brownout` does NOT reset warmth/emit `cache_cold`, clearing an actual `down` fault still does. |
| `src/lib/worldEngine/replication.ts` | FEAT-005 | Pure, dependency-free lag/backlog/RPO resolver (`localityFloorSec`/`stepReplication`/`staleReadFraction`, plus `createReplicationState`), imports only `interRegionLatencyMs` from `regionConfig.ts`. Consumed from `index.ts`'s `runStep`: `stepReplication` mutates the persistent `EngineState.replication` (backlog/lag maps) once per step, after the flow solve, using a per-step `applyCapacity` derived from either `DbConfig.applyRatePerReplica` or the replica's own resolved service rate × `WRITE_APPLY_EFFICIENCY_CONST`. `staleReadFraction` is read one-step-lagged (this step's stale-read signal is built from the *previous* step's write-rps/lag, since this step's own write-rps is only known after the same `solveFlows` call it needs to feed) and threaded into `flows.ts` as `FlowInput.staleReadFractionByReplica`, attached to `DownstreamFlow.staleReadFraction` only on rows landing on an effective-role `'replica'`. Introduced: four optional `DbConfig` fields (Task 9), `MetricsBatch.clusters?: Record<string, {lagSec}>` and `InstanceMetrics.staleReadFraction?: number` (Task 12, max-lag-across-replicas per cluster), `replica_promoted`'s `payload.dataLossWindowSec`/`estimatedLostWrites` (Task 13, `promoteReplicas` now also accepts optional `lagByInstance`/`writeRpsByReplica` params that change candidate-selection tiebreak to prefer least-lagged), `replication_lag_high`/`stale_read_served` events (Task 14, rate-limited via the same `Map<key, lastEmittedAtSimMs>` shape `connection_refused` already uses). Start-time index: `EngineState.replicasByCluster` (built once in `buildReplicationIndexes`, cluster key `${primary.blueprintId}|${primary.regionId}` — matching `failover.ts`'s own promotion-clustering convention exactly, keyed by the PRIMARY's blueprint+region so a primary's same-region and cross-region replicas share one write stream), `EngineState.hasAnyReplicas` (fast-path guard). `src/lib/world/replicaLag.ts` (Task 15, UI-side) intentionally duplicates `buildReplicationIndexes`' cluster-key/primary-tiebreak convention for a read-only display lookup — it does not re-derive the lag value (always read from the engine-published `MetricsBatch.clusters`), but the key-derivation logic itself is a second copy; if the engine's tiebreak convention ever changes, this file must change with it (flagged in its own header comment, covered by `replicaLag.test.ts`). |
...
- `src/lib/worldEngine/hostScheduler.ts` — gains disk-I/O demand and wait-time functions (FEAT-006, no new file): `diskIoDemandFor(loads, blueprintByInstance, doc)` sums `admittedRps * workload.diskIoPerRequest` across a server's resident instances; `diskWaitFor(demandIops, diskIops, diskType)` shares `poolCheckoutFor`'s `base/(1-min(overshoot,0.98))` queueing curve verbatim (`BASE_DISK_MS` per type: hdd 8/ssd 0.5/nvme 0.1; `DEFAULT_DISK_IOPS`: hdd 150/ssd 16,000/nvme 100,000), returning `null` when neither `diskIops` nor `diskType` is resolvable — the same "not capacity-modelled" convention `poolCheckoutFor`/`managedDbRuntimeFor` already share. `resolveDiskIopsCeiling` is the shared ceiling-resolution helper both `diskWaitFor` and `index.ts`'s `diskIoRatio` computation call, so the two never diverge. `HostStepResult.diskWaitMsByInstance?`/`diskIoRatio?` are new additive-optional fields (Task 19/20), the latter feeding `metrics.ts`'s dual-behavior `ServerMetrics.diskIoFraction` (legacy `diskIo/100` heuristic preserved byte-identical when no ceiling is authored; `min(1, demandIops/ceiling)` when one is — both `disk_saturated`'s engine-event gate and the `iops-saturated` analysis rule check ceiling-resolvability before treating the fraction as meaningful, after a review-caught false-positive on the legacy branch was fixed in Task 21). `extraLatencyMsByServer` in `index.ts` folds `diskWaitMs` in ADDITIVELY alongside NIC-queue latency and any `latency-add` fault (never assigned) — this is the SOLE mechanism by which disk wait reaches both Little's-law RAM call sites (scheduler-side `InstanceLoad.activeConnections` and published `activeConnections`), both of which already read the same `f.totalLatencyMs`/`prevFlows` pipeline `latency-add` established in Wave 1; there is deliberately no second, same-step RAM injection point, since one would double-count the same latency term. `disk-stall` (`FaultSpec`'s sixth variant, Task 17) multiplies effective `diskIops` the same way `cpu-brownout` multiplies `effectiveVcpu` — composed, never replacing. `managedDbRuntimeFor` (`src/lib/managedDbRuntime.ts`) gained a third `iopsUtilization` saturation axis from `ManagedService.provisionedIops`, composed into its existing `Math.max(writeUtil, readUtil, iopsUtilization)` — the first behavioral (not just cost-model) consequence of that field. **Audit final-review fix**: unlike FEAT-004/005, FEAT-006 originally shipped `index.ts`'s per-server per-step disk block (the `diskIoDemandFor`/`diskWaitFor`/`resolveDiskIopsCeiling` calls, plus a fresh `new Map(resident.map(...))` blueprint-lookup allocation) with NO `hasAnyCache`/`hasAnyReplicas`-style fast-path guard — it ran unconditionally every server every step. Since Task 17 gives every `instanceCatalog.ts` preset a default `diskType`, an unconfigured-by-preset world is now effectively always on this path, defeating the intended zero-cost-when-unused property. Added `EngineState.hasAnyDisk` (computed once at `start()`, `Object.values(doc.servers).some(sv => sv.specs.diskIops != null || sv.specs.diskType != null)`, the SAME "authored" test `diskWaitFor`/`resolveDiskIopsCeiling`/`iopsSaturated` already use) gating the whole block, plus `EngineState.blueprintIdByInstance` — a start()-time `Map<InstanceId, BlueprintId>` built once from `compiled.instances` — replacing the per-server per-step `Map` allocation `diskIoDemandFor` needs. Only a world with NEITHER field authored anywhere (hand-built `ServerSpecs`, or a pre-Task-17 saved world never re-touched) now pays zero per-step disk cost; a preset-derived world (the common case) is unaffected since `diskType` IS the authored signal there.
...
- `src/app/world/server/HardwarePlatform.tsx` — the disk platter's existing "nvme0 · io NN%" line
  ...
  `DATA_LOSS_DANGER_FALLBACK_SEC` mirrors an analysis-rule threshold above). **Audit final-review
  fix**: Task 22 originally applied the threshold unconditionally, redlining on `diskIoFraction`'s
  legacy `min(1, diskIo/100)` fallback for any server with NEITHER `diskIops` nor `diskType`
  authored — the exact false-positive shape `iopsSaturated` had ALREADY been fixed against in Task
  21. The bar now computes `hasDiskCeiling = server.specs.diskIops != null || server.specs.diskType
  != null` (the SAME gate `iopsSaturated` uses) and only redlines when both `hasDiskCeiling` and
  the threshold are true (`disk-io-fill`'s `data-saturated` attribute reflects the gated result,
  not the raw fraction) — restoring the "board and `iops-saturated` never disagree" invariant this
  section originally (incorrectly) claimed was already true. Covered by two new
  `HardwarePlatform.test.tsx` cases: no redline at `diskIoFraction=0.95` with neither field
  authored, redline still fires with `diskType` authored. The platter's existing spin animation
  (`platter-spin`, spin duration ∝ `diskIoFraction`) is unchanged — the new bar is an additional,
  more literal "how close to the ceiling" readout alongside it, not a replacement.
