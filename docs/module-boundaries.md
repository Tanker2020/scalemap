...
| `src/app/canvas/nodes/BaseNode.tsx` | Compute/network/storage node rendering — **known gap:** `statusColor`/`saturationBorderColor`/`utilColor`/`lintColor` (lines ~69-92) hardcode dark-mode-tuned status hexes (`#22C55E`/`#F59E0B`/`#EF4444`) inline via `style={{}}`/CSS custom-property values, bypassing `BaseNode.module.css` entirely for `.statusDot`, `.utilFill`, and the `--lint-color` fallback consumer; `BaseNode.module.css`'s own severity selectors (`.saturated`/`.critical`/circuit-breaker/badges) were fixed to use `var(--color-warning/danger/success)` (design-system task 10 follow-up), but these four `.tsx`-side inline values were out of that fix's file scope and still need the same `themeMode`-aware treatment (the component already reads `themeMode` at line 26 for `accentColor`, so the fix is the same pattern) |
