...
| `src/app/dock/UtilityDock.module.css` | Tab strip + shell styling; `.dock` is `position: fixed; right: 16px; bottom: 44px`, `width: 340px`, `max-height: min(480px, calc(100vh - 42px - 28px - 32px))` (capped so it can never grow tall enough to reach the top-right Properties/SimConfig card), `z-index: 500`, rounded card with border + shadow |
